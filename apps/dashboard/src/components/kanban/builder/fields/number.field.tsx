...
import { Binary, Calculator, Text } from "lucide-react"
...
const type: FieldsType = 'NumberField'
...
const FieldComponent = ({ fieldInstance, submitValue, isInvalid, defaultValue }: FieldComponentProps) => {
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            ...
            <Input
                ...
                onBlur={(e) => {
                    const valid = NumberFieldForm.validate(element, e.target.value)
                    setError(!valid)
                    if (!valid) return
                    submitValue?.(element._id, e.target.value)
                }}
            />
...
const DesignerComponent = ({ fieldInstance }: { fieldInstance: FormFieldInstance }) => {
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            ...
            <Input type="number" readOnly disabled placeholder={element.extraAttributes.placeholder} />
            {element.extraAttributes.description && <p className="text-muted-foreground text-[0.8rem]">{element.extraAttributes.description}</p>}
        </div>
    )
}

export const NumberFieldForm: FormElement = {
    type,
