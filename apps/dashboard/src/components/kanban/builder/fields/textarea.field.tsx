...
import { LoaderIcon, Text } from "lucide-react"
...
const type: FieldsType = 'TextAreaField'
...
const FieldComponent = ({ fieldInstance, submitValue, isInvalid, defaultValue, promise, ...props }: FieldComponentProps) => {
    ...
    const handlerBlur = (event: FocusEvent<HTMLTextAreaElement>) => {
        const valid = TextareaForm.validate(element, event.target.value)
        setError(!valid)
        if (!valid) return
...
const DesignerComponent = ({ fieldInstance }: { fieldInstance: FormFieldInstance }) => {
    ...
    const element = fieldInstance as CustomInstance
    const { label, required, placeholder, description, rows } = element.extraAttributes
    return (
        <div className="flex flex-col gap-2 w-full">
            <Label>
                {label}
...
export const TextareaForm: FormElement = {
    type,
