...
//#region node_modules/.nitro/vite/services/ssr/assets/stations-BkSUnpAK.js
...
var SQL = class SQL {
	...
	get usedTables() {
		...
		for (let i = 0; i < this.queryChunks.length; ++i) {
			...
			if (is(chunk, Table)) {
				...
				this._usedTables.push(schemaName === void 0 ? chunk[Table.Symbol.Name] : schemaName + "." + chunk[Table.Symbol.Name]);
			}
		}
	...
	buildQueryFromSourceParams(chunks, _config) {
		const strings = [];
		const params = [];
		const bounds = _config.tagged ? [] : void 0;
		this.collectSQL(chunks, _config, _config.paramStartIndex || { value: 0 }, _config.inlineParams || this.shouldInlineParams, strings, params, bounds);
		if (bounds === void 0) return {
			sql: strings.join(""),
			params
		};
		const segments = new Array(bounds.length);
	...
	collectSQL(chunks, config, paramStartIndex, inlineParams, strings, params, bounds) {
		const { escapeName, escapeParam, codecs, invokeSource } = config;
		for (let i = 0; i < chunks.length; ++i) {
			if (bounds !== void 0) bounds.push(strings.length);
			...
			if (is(chunk, SQL)) {
				this.collectSQL(chunk.queryChunks, config, paramStartIndex, inlineParams || chunk.shouldInlineParams, strings, params, void 0);
				continue;
			}
			if (is(chunk, Name)) {
			...
			if (is(chunk, Column)) {
				...
				strings.push(chunk.isAlias ? escapeName(chunk.name) : chunk.table[IsAlias] || schemaName === void 0 ? escapeName(chunk.table[Table.Symbol.Name]) + "." + escapeName(columnName) : escapeName(schemaName) + "." + escapeName(chunk.table[Table.Symbol.Name]) + "." + escapeName(columnName));
				continue;
			}
			if (is(chunk, Param)) {
				if (is(chunk.value, SQL)) {
					const nested = chunk.value;
					this.collectSQL(nested.queryChunks, config, paramStartIndex, inlineParams || nested.shouldInlineParams, strings, params, void 0);
					continue;
				}
				...
				if (is(chunk.value, Placeholder)) {
					...
					strings.push(useCodecs ? codecs.apply(chunk.encoder, "castParam", escaped) : escaped);
					params.push(chunk);
					continue;
				}
				let mappedValue;
			...
			if (is(chunk, Table)) {
				...
				if (invokeSource === "mssql-view-with-schemabinding") {
					strings.push((schemaName === void 0 ? escapeName("dbo") : escapeName(schemaName)) + "." + escapeName(tableName));
					continue;
				}
				strings.push(schemaName === void 0 || chunk[IsAlias] ? escapeName(tableName) : escapeName(schemaName) + "." + escapeName(tableName));
				continue;
			}
			if (is(chunk, View)) {
				const schemaName = chunk[ViewBaseConfig].schema;
				const viewName = chunk[ViewBaseConfig].name;
				strings.push(schemaName === void 0 || chunk[ViewBaseConfig].isAlias ? escapeName(viewName) : escapeName(schemaName) + "." + escapeName(viewName));
				continue;
			}
			if (is(chunk, SQL.Aliased) && chunk.fieldAlias !== void 0) {
				strings.push((chunk.origin !== void 0 ? escapeName(chunk.origin) + "." : "") + escapeName(chunk.fieldAlias));
				continue;
			}
			...
			if (typeof chunk === "function" && "enumName" in chunk) {
				if ("schema" in chunk && chunk.schema) {
					strings.push(escapeName(chunk.schema) + "." + escapeName(chunk.enumName));
					continue;
				}
				strings.push(escapeName(chunk.enumName));
				continue;
			}
			if (isSQLWrapper(chunk)) {
				...
				if (chunk.shouldOmitSQLParens?.()) {
					this.collectSQL(nested.queryChunks, config, paramStartIndex, nestedInlineParams, strings, params, void 0);
					continue;
				}
				strings.push("(");
				this.collectSQL(nested.queryChunks, config, paramStartIndex, nestedInlineParams, strings, params, void 0);
			...
			if (Array.isArray(chunk)) {
				...
				for (let j = 0; j < chunk.length; ++j) {
					if (j > 0) strings.push(", ");
					element[0] = chunk[j];
					this.collectSQL(element, config, paramStartIndex, inlineParams, strings, params, void 0);
				}
				strings.push(")");
				continue;
			}
...
function sql(strings, ...params) {
	...
	const queryChunks = new Array(chunkCount < 0 ? 0 : chunkCount);
	let writeIdx = 0;
	if (startWithString) queryChunks[writeIdx++] = new StringChunk(strings[0]);
...
(function(_sql) {
	...
	function join(chunks, separator) {
		const { length } = chunks;
		if (separator === void 0 || length === 0) return new SQL(chunks.slice());
		const result = new Array(length * 2 - 1);
		for (let i = 0, writeIdx = 0; i < length; ++i) {
			result[writeIdx++] = chunks[i];
			if (i < length - 1) result[writeIdx++] = separator;
		}
...
var ColumnTableAliasProxyHandler = class {
	...
	get(columnObj, prop) {
		...
		if (prop === OriginalColumn && !this.ignoreColumnAlias) return () => columnObj;
...
function orderSelectedFields(fields, pathPrefix, codecs, result = []) {
	...
	for (let i = 0; i < entries.length; ++i) {
		...
		} else if (is(field, Table)) orderSelectedFields(field[Table.Symbol.Columns], newPath, codecs, result);
...
function mapUpdateSet(table, values) {
	...
	for (let i = 0; i < entries.length; ++i) {
		...
		if (is(value, SQL) || is(value, Column)) mapped[i] = [key, value];
		else mapped[i] = [key, new Param(value, table[Table.Symbol.Columns][key])];
	}
...
function processRelations(tablesConfig, tables) {
	...
	for (const [sourceTableName, tableConfig] of Object.entries(tablesConfig)) for (const [relationFieldName, relation] of Object.entries(tableConfig.relations)) {
		...
		if (Object.prototype.hasOwnProperty.call(tableConfig.table[TableColumns], relationFieldName)) throw new Error(`${relationPrintName}: relation name collides with column "${relationFieldName}" of table "${tableConfig.name}"`);
...
var One = class extends Relation {
	...
	constructor(tables, targetTable, targetTableName, config) {
		...
		if (config && "where" in config) {
			if (config.where === void 0) throw new Error(`Unexpected 'undefined' in filter value. Use 'EmptyFilter' if you want the filter field to be skipped.`);
			this.where = config.where;
		} else this.where = EmptyFilter;
...
function mapRelationalRow(rows, isOne, buildQueryResultSelection, parseJson = false, parseJsonIfString = false, useJsonMappers = true) {
	...
	const decoders = buildQueryResultSelection.map(({ field, fieldType, codec, arrayDimensions }) => {
		let decoder;
		switch (fieldType) {
			case "Column":
				decoder = field;
			...
			case "Nested":
				decoder = noopDecoder;
				break;
			default: decoder = field.getSQL().decoder;
		}
...
function mapRelationalRowFromArrays(rows, isOne, buildQueryResultSelection, parseJson = false, parseJsonIfString = false) {
	...
	const results = new Array(maxIdx);
	for (let i = 0; i < maxIdx; ++i) {
		const row = isOne ? rows : rows[i];
		const result = {};
		for (let selectionItemIdx = 0; selectionItemIdx < buildQueryResultSelection.length; ++selectionItemIdx) {
...
function makeJitRqbMapperInner(selection, rowExpr, selectionVar, parseJson, parseJsonIfString, useJsonMappers, preFn, counter, accessByIdx) {
	...
	for (const [idx, { field, fieldType, key, codec, isArray, selection: innerSelection, arrayDimensions }] of selection.entries()) {
...
function relationsFieldFilterToSQL(column, filter) {
	if (filter === EmptyFilter) return void 0;
	if (filter === void 0) throw new Error(`Unexpected 'undefined' in filter value. Use 'EmptyFilter' if you want the filter field to be skipped.`);
	...
	for (const [target, value] of entries) {
		...
		if (value === void 0) throw new Error(`Unexpected 'undefined' in filter value. Use 'EmptyFilter' if you want the filter field to be skipped.`);
		switch (target) {
			case "NOT": {
			...
			case "OR":
				if (!value.length) throw new Error("Unexpected empty array in filters' 'OR' section. Omit field or use 'EmptyFilter' if you want filter to be skipped.");
				parts.push(or(...value.map((subFilter) => relationsFieldFilterToSQL(column, subFilter))));
				continue;
			case "AND":
				if (!value.length) throw new Error("Unexpected empty array in filters' 'AND' section. Omit field or use 'EmptyFilter' if you want filter to be skipped.");
	...
	for (const [target, value] of entries) {
		...
		switch (target) {
			...
			default: {
				...
				const relation = Object.prototype.hasOwnProperty.call(tableRelations, target) ? tableRelations[target] : void 0;
...
function relationsOrderToSQL(table, orders) {
	if (typeof orders === "function") {
		...
		return is(data, SQL) ? data : Array.isArray(data) ? data.length ? sql.join(data.map((o) => is(o, SQL) ? o : asc(o)), new StringChunk(`, `)) : void 0 : is(data, Column) ? asc(data) : void 0;
	}
	const entries = Object.entries(orders).filter(([_, value]) => value);
	if (!entries.length) return void 0;
	return sql.join(entries.map(([target, value]) => (value === "asc" ? asc : desc)(fieldSelectionToSQL(table, target))), new StringChunk(`, `));
}
function relationExtrasToSQL(table, extras, codecs, inJson) {
	...
	return {
		sql: subqueries.length ? sql.join(subqueries, new StringChunk(`, `)) : void 0,
		selection
	};
}
function relationToSQL(relation, sourceTable, targetTable, throughTable) {
	if (relation.through) {
		...
		return {
			filter: and(relationsFilterToSQL(relation.isFilterReversed ? sourceTable : targetTable, relation.where), ...outerColumnWhere),
...
var EmptyFilter = Symbol.for("drizzle:EmptyFilter");
	...
	toBuilder(overrides) {
		...
		if (overrides) {
			...
			if (unique !== void 0) {
				...
				const uniqueConfig = typeof unique === "object" ? unique : void 0;
				config.uniqueName = uniqueConfig?.name;
				config.uniqueType = uniqueConfig?.nulls;
			}
			config.hasDefault = config.default !== void 0 || config.defaultFn !== void 0 || config.onUpdateFn !== void 0 || config.generatedIdentity !== void 0;
		}
		return new PgClonedColumnBuilder(config, Object.getPrototypeOf(this).constructor);
	}
	/** @internal */
	shouldDisableInsert() {
...
var PgClonedColumnBuilder = class extends PgColumnBuilder {
	static [entityKind] = "PgClonedColumnBuilder";
	ColumnClass;
	constructor(config, ColumnClass) {
		super(config.name, config.dataType, config.columnType);
		this.config = config;
...
var DEFAULT_DELIM = ",";
function parsePgArrayValue(arrayString, startFrom, inQuotes, delim) {
	for (let i = startFrom; i < arrayString.length; i++) {
		...
		if (char === "\"") return [arrayString.slice(startFrom, i).replace(/\\(.)/g, "$1"), i + 1];
		if (inQuotes) continue;
		if (char === delim || char === "}") return [arrayString.slice(startFrom, i).replace(/\\(.)/g, "$1"), i];
	}
	return [arrayString.slice(startFrom).replace(/\\(.)/g, "$1"), arrayString.length];
}
function parsePgNestedArray(arrayString, startFrom = 0, delim = DEFAULT_DELIM) {
	const result = [];
	let i = startFrom;
	...
	while (i < arrayString.length) {
		const char = arrayString[i];
		if (char === delim) {
			if (lastCharIsDelim || i === startFrom) result.push("");
...
function parsePgArray(arrayString, delim = DEFAULT_DELIM) {
	const [result] = parsePgNestedArray(arrayString, 1, delim);
	return result;
}
function buildPgArray(array, delim) {
	return `{${array.map((item) => {
		if (Array.isArray(item)) return buildPgArray(item, delim);
		if (typeof item === "string") return `"${item.replace(/\\/g, "\\\\").replace(/"/g, "\\\"")}"`;
		return `${item}`;
	}).join(delim)}}`;
}
function makePgArray(array, delim = DEFAULT_DELIM) {
...
var parsePgArrayAndNormalize = (normalize, delim) => {
	const codec = arrayCompatNormalize(normalize);
	return (value, arrayDimensions) => codec(parsePgArray(value, delim), arrayDimensions);
};
var parseGeometryArrayAndNormalize = (normalize) => parsePgArrayAndNormalize(normalize, ":");
var makeGeometryArray = (value) => makePgArray(value, ":");
...
var PgSelectBase = class extends TypedQueryBuilder {
	...
	*/
	_resolveSelection() {
		...
		config.fieldsFlat ??= orderSelectedFields(config.fields, void 0, dialect.codecs);
		const { fieldsFlat, setOperators } = config;
		if (setOperators.length && !config.setFieldsFlat) {
			const setSelection = new Array(fieldsFlat.length);
			for (let i = 0; i < setOperators.length; ++i) {
				...
				for (let j = 0; j < fieldsFlat.length; ++j) {
					...
					setSelection[j].codecOverride = lc && rc ? unionsTypeTable[lc]?.[rc] : lc;
				}
			}
			for (let i = 0; i < setSelection.length; ++i) {
				const out = setSelection[i];
				out.codec = out.codecOverride ? dialect.codecs.get(out.column, "normalize", out.codecOverride) : out.codec;
			}
...
var PgDialect = class {
	...
	*/
		...
		for (let i = 0; i < columnsLen; ++i) {
			...
			switch (fieldType) {
				...
				case "SQL":
					...
					break;
				case "Subquery": if (column && !ignoreCastCodecs && !field._.isWith) {
	...
	buildSelectQuery({ withList, fieldsFlat, where, having, table, joins, orderBy, groupBy, limit, offset, lockingClause, distinct, setOperators, setFieldsFlat: setSelection, comment, ignoreSelectionCastCodecs }) {
		...
		const withSql = this.buildWithCTE(withList);
		let distinctSql;
		if (distinct) distinctSql = distinct === true ? sql` distinct` : sql` distinct on (${sql.join(distinct.on, new StringChunk(", "))})`;
		...
		let orderBySql;
		if (orderBy && orderBy.length > 0) orderBySql = sql` order by ${sql.join(orderBy, new StringChunk(", "))}`;
		let groupBySql;
		if (groupBy && groupBy.length > 0) groupBySql = sql` group by ${sql.join(groupBy, new StringChunk(", "))}`;
		const limitSql = typeof limit === "object" || typeof limit === "number" && limit >= 0 ? sql` limit ${limit}` : void 0;
		const offsetSql = offset ? sql` offset ${offset}` : void 0;
		const lockingClauseSql = sql.empty();
		if (lockingClause) {
			const clauseSql = sql` for ${new StringChunk(lockingClause.strength)}`;
			if (lockingClause.config.of) clauseSql.append(sql` of ${sql.join(Array.isArray(lockingClause.config.of) ? lockingClause.config.of.map((it) => sql.identifier(it[PgTable.Symbol.Name])) : [sql.identifier(lockingClause.config.of[PgTable.Symbol.Name])], new StringChunk(", "))}`);
		...
		if (select) valuesSqlList[0] = valuesOrSelect.getSQL();
			...
			for (let valueIndex = 0; valueIndex < values.length; ++valueIndex) {
				...
				for (let i = 0; i < colFilteredEntries.length; ++i) {
					...
					if (i < colFilteredEntries.length - 1) valuesSqlList[writeIdx++] = new StringChunk(", ");
				}
				valuesSqlList[writeIdx++] = new StringChunk(")");
	...
	buildRqbColumn(table, field, key, inJson, selection, tableTsName) {
		...
		if (is(field, Column)) {
			...
			output = sql`${inJson && field.jsonSelectIdentifier ? field.jsonSelectIdentifier(name, sql, field.dimensions) : this.codecs.apply(field, inJson ? "castInJson" : "cast", name)} as ${sql.identifier(key)}`;
		} else if (is(field, SQL)) {
	...
	buildRelationalQuery({ schema, table, tableConfig, queryConfig: config, relationWhere, mode, errorPath, depth, throughJoin, nested }) {
		...
		switch (params) {
			...
			default: {
				...
				for (let readIdx = 0, writeIdx = 1; readIdx < withEntries.length; ++readIdx) {
					...
					const joinQuery = sql`left join lateral(select ${isSingle ? sql`row_to_json(${sql.identifier("t")}.*) ${sql.identifier("r")}` : sql`coalesce(json_agg(row_to_json(${sql.identifier("t")}.*)), '[]') as ${sql.identifier("r")}`} from (${innerQuery.sql}) as ${sql.identifier("t")}) as ${sql.identifier(k)} on true`;
		...
		return {
			sql: sql`select ${selectionSet} from ${getTableAsAliasSQL(table)}${throughJoin}${joins}${where ? sql` where ${where}` : void 0}${order ? sql` order by ${order}` : void 0}${limit !== void 0 ? sql` limit ${limit}` : void 0}${offset !== void 0 ? sql` offset ${offset}` : void 0}${comment ? sql` ${comment}` : void 0}`,
			selection
		};
	}
};
...
var PgAsyncDeleteBase = class extends PgDeleteBase {
	...
	_prepare(name, generateName = false) {
		...
		return tracer.startActiveSpan("drizzle.prepareQuery", () => {
			const shape = fields ? config.shape ??= dialect.shapeGenerator?.({
				type: "plain",
				fields
			}, void 0) : void 0;
			const query = dialect.sqlToQuery(this.getSQL());
			const mapper = shape || !fields ? void 0 : this.dialect.mapperGenerators.rows(fields, void 0);
			return session.prepareQuery(query, shape ? "objects" : fields ? "arrays" : "raw", name ?? generateName, mapper, {
				type: "delete",
...
var PgInsertBuilder = class {
	...
	constructor(table, session, dialect, withList, overridingSystemValue_, columnList, builder = PgInsertBase) {
		this.table = table;
		this.session = session;
		this.dialect = dialect;
	...
	values(values) {
		...
		return new this.builder(this.table, values, this.session, this.dialect, this.withList, false, this.columnList, this.overridingSystemValue_);
	}
	select(selectQuery) {
		...
		return new this.builder(this.table, select, this.session, this.dialect, this.withList, true, this.columnList, this.overridingSystemValue_);
	}
};
var PgInsertBase = class {
	static [entityKind] = "PgInsert";
	config;
	constructor(table, values, session, dialect, withList, select, columnList, overridingSystemValue_) {
		this.session = session;
		this.dialect = dialect;
		this.config = {
			table,
	...
	_prepare(name, generateName = false) {
		return tracer.startActiveSpan("drizzle.prepareQuery", () => {
			...
			const shape = dialect.shapeGenerator?.({
				type: "relational",
				fields: query.selection
			}, void 0);
			const mapper = shape ? isFirst ? (rows) => rows[0] : void 0 : dialect.mapperGenerators.relationalRows({
				isFirst,
			...
			return this.session.prepareQuery(builtQuery, shape ? "objects" : "arrays", name ?? generateName, mapper, void 0, void 0, shape);
		});
	}
	prepare(name) {
		return this._prepare(name, true);
...
var PgAsyncSelectBase = class extends PgSelectBase {
	...
	_prepare(name, generateName = false) {
		...
		return tracer.startActiveSpan("drizzle.prepareQuery", () => {
			...
			const mapper = shape ? void 0 : dialect.mapperGenerators.rows(fieldsList, nullableObjectPaths);
			return session.prepareQuery(query, shape ? "objects" : "arrays", name ?? generateName, mapper, {
				type: "select",
				tables: [...usedTables]
			}, cacheConfig, shape);
		});
...
var PgAsyncUpdateBase = class extends PgUpdateBase {
	...
	_prepare(name, generateName = false) {
		...
		const { returning: fields } = config;
		return tracer.startActiveSpan("drizzle.prepareQuery", () => {
			const nullableObjectPaths = fields ? resolveNullableObjectPaths(fields, joinsNotNullableMap) : void 0;
			const shape = fields ? config.shape ??= dialect.shapeGenerator?.({
				type: "plain",
				fields
			}, nullableObjectPaths) : void 0;
			if (shape) this.withoutSelectionCastCodecs();
			const query = dialect.sqlToQuery(this.getSQL());
			const mapper = shape || !fields ? void 0 : this.dialect.mapperGenerators.rows(fields, nullableObjectPaths);
			return session.prepareQuery(query, shape ? "objects" : fields ? "arrays" : "raw", name ?? generateName, mapper, {
				type: "update",
				tables: [...extractUsedTable(this.config.table)]
			}, void 0, shape);
...
var PgAsyncDatabase = class {
	...
	with(...queries) {
		...
		*/
		...
		function insert(table, ...columns) {
			return new PgInsertBuilder(table, self.session, self.dialect, queries, void 0, columns.length ? columns : void 0, PgAsyncInsertBase);
		}
		/**
	...
	*/
	...
	insert(table, ...columns) {
		return new PgInsertBuilder(table, this.session, this.dialect, void 0, void 0, columns.length ? columns : void 0, PgAsyncInsertBase);
	}
	/**
	* Creates a delete query.
	*
	...
	*/
	...
	execute(query, mode) {
		...
		return new PgAsyncRaw(this.session.prepareQuery(builtQuery, mode ?? "raw", false), sequel, builtQuery);
	}
...
var PgAsyncTransaction = class extends PgAsyncDatabase {
	...
	/** @internal */
	getTransactionConfigChunks(config) {
		const chunks = [];
	...
	/** @internal */
	setTransactionSnapshotSQL(snapshot) {
		return sql`set transaction snapshot ${snapshot}`.inlineParams();
	}
	async setTransaction(config) {
		const chunks = this.getTransactionConfigChunks(config);
		if (chunks.length) await this.session.execute(sql.raw(`set transaction ${chunks.join(" ")}`));
		if (typeof config.snapshot === "string") return this.session.execute(this.setTransactionSnapshotSQL(config.snapshot));
	}
};
var tables_exports = /* @__PURE__ */ __exportAll({
	account: () => account,
	session: () => session,
	user: () => user,
...
var verification = pgTable("verifications", {
	id: uuid("id").primaryKey().defaultRandom(),
...
var equipmentModels = pgTable("equipment_models", {
	id: uuid("id").primaryKey().defaultRandom(),
	manufacturer: text("manufacturer").notNull(),
	model: text("model").notNull(),
	description: text("description"),
	createdAt: timestamp("created_at").notNull().defaultNow()
}, (table) => [uniqueIndex("equipment_models_manufacturer_model_unique").on(table.manufacturer, table.model)]);
var equipments = pgTable("equipments", {
	...
	uniqueIndex("equipment_customer_serial_unique").on(table.customerId, table.sn),
	uniqueIndex("equipment_customer_asset_unique").on(table.customerId, table.assetNumber)
]);
var products = pgTable("products", {
	...
	updatedById: uuid("updated_by_id").notNull().references(() => user.id),
	createdAt: timestamp("created_at").notNull().defaultNow(),
...
var worksToItems = pgTable("works_to_items", {
...
var sells = pgTable("sells", {
	id: uuid("id").primaryKey().defaultRandom(),
	orderId: uuid("order_id").notNull().references(() => orders.id)
});
var channelModeEnum = pgEnum("channel_mode", ["ANALOG", "DIGITAL"]);
var digitalProtocolEnum = pgEnum("digital_protocol", [
	"DMR",
	"P25",
	"DSTAR",
	"OTHER"
]);
var analogSquelchModeEnum = pgEnum("analog_squelch_mode", [
	"CARRIER",
	"CTCSS",
	"DCS"
]);
var stationModeEnum = pgEnum("station_mode", [
	"ANALOG",
	"DIGITAL",
	"MIXED"
]);
var stations = pgTable("stations", {
	...
	createdAt: timestamp("created_at").notNull().defaultNow(),
	updatedAt: timestamp("updated_at").notNull().defaultNow()
}, (table) => [index("stations_customer_idx").on(table.customerId), index("stations_location_idx").on(table.latitude, table.longitude)]);
var stationVersions = pgTable("station_versions", {
	id: uuid("id").primaryKey().defaultRandom(),
	...
	reason: text("reason")
}, (table) => [uniqueIndex("station_versions_unique").on(table.stationId, table.version), index("station_versions_station_idx").on(table.stationId)]);
var channels = pgTable("channels", {
	...
	createdAt: timestamp("created_at").notNull().defaultNow(),
	updatedAt: timestamp("updated_at").notNull().defaultNow()
}, (table) => [index("channels_customer_idx").on(table.customerId), index("channels_frequency_idx").on(table.rx, table.tx)]);
var digitalChannels = pgTable("channel_digital", {
...
var analogChannels = pgTable("channel_analogs", {
	channelId: uuid("channel_id").primaryKey().references(() => channels.id),
	squelchMode: analogSquelchModeEnum("squelch_mode").notNull().default("CARRIER"),
...
var channelGroups = pgTable("channel_groups", {
...
var talkgroupTypeEnum = pgEnum("talkgroup_type", [
	"GROUP",
	"PRIVATE",
	"ALL_CALL"
]);
var talkgroups = pgTable("talkgroups", {
	id: uuid("id").primaryKey().defaultRandom(),
	customerId: uuid("customer_id").notNull().references(() => customers.id),
	...
	createdAt: timestamp("created_at").notNull().defaultNow(),
	updatedAt: timestamp("updated_at").notNull().defaultNow()
}, (table) => [uniqueIndex("talkgroups_customer_number_unique").on(table.customerId, table.number)]);
var programmingStatusEnum = pgEnum("programming_status", [
	"DRAFT",
	"ACTIVE",
	"ARCHIVED"
]);
var programmingProfiles = pgTable("programming_profiles", {
	id: uuid().primaryKey().defaultRandom(),
...
}, (table) => [uniqueIndex("programming_profiles_equipment_version_unique").on(table.equipmentId, table.version), index("programming_profiles_equipment_idx").on(table.equipmentId)]);
var programmingChannels = pgTable("programming_channels", {
	profileId: uuid("profile_id").notNull().references(() => programmingProfiles.id),
	...
	createdAt: timestamp("created_at").notNull().defaultNow()
}, (table) => [primaryKey({ columns: [table.profileId, table.channelId] }), index("programming_channels_channel_idx").on(table.channelId)]);
var programmingGroups = pgTable("programming_groups", {
	profileId: uuid("profile_id").notNull().references(() => programmingProfiles.id),
	groupId: uuid("group_id").notNull().references(() => channelGroups.id),
	position: integer("position"),
	createdAt: timestamp("created_at").notNull().defaultNow()
}, (table) => [primaryKey({ columns: [table.profileId, table.groupId] })]);
var stationChannels = pgTable("station_channels", {
	stationId: uuid("station_id").notNull().references(() => stations.id),
	channelId: uuid("channel_id").notNull().references(() => channels.id),
	position: integer("position"),
	alias: text("alias"),
	...
	createdAt: timestamp("created_at").notNull().defaultNow()
}, (table) => [primaryKey({ columns: [table.stationId, table.channelId] }), index("station_channels_channel_idx").on(table.channelId)]);
var channelGroupChannels = pgTable("channel_group_channels", {
	groupId: uuid("group_id").notNull().references(() => channelGroups.id),
	channelId: uuid("channel_id").notNull().references(() => channels.id),
	position: integer(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow()
}, (table) => [primaryKey({ columns: [table.groupId, table.channelId] })]);
var db_exports = /* @__PURE__ */ __exportAll({
	account: () => account,
	analogChannels: () => analogChannels,
	analogSquelchModeEnum: () => analogSquelchModeEnum,
	channelGroupChannels: () => channelGroupChannels,
	channelGroups: () => channelGroups,
	channelModeEnum: () => channelModeEnum,
	channels: () => channels,
	customers: () => customers,
	digitalChannels: () => digitalChannels,
	digitalProtocolEnum: () => digitalProtocolEnum,
	...
	programmingGroups: () => programmingGroups,
	programmingProfiles: () => programmingProfiles,
	programmingStatusEnum: () => programmingStatusEnum,
	sells: () => sells,
	session: () => session,
	stationChannels: () => stationChannels,
	stationModeEnum: () => stationModeEnum,
	stationVersions: () => stationVersions,
	stations: () => stations,
	talkgroupTypeEnum: () => talkgroupTypeEnum,
	talkgroups: () => talkgroups,
	user: () => user,
	verification: () => verification,
	works: () => works,
	worksToItems: () => worksToItems
});
...
var nodePgCodecs = refineGenericPgCodecs({
	...
	bit: { normalizeArray: (v) => parsePgArray(v) },
	numeric: { castArray: castToTextArr },
	"numeric:number": {
	...
	sparsevec: { normalizeArray: (v) => parsePgArray(v) },
	vector: {
		normalize: parsePgVector,
		normalizeArray: parsePgArrayAndNormalize(parsePgVector)
	}
...
defineRelationsPart(tables_exports, (r) => ({
	user: {
...
var main = defineRelations(db_exports, (r) => ({ orders: { works: r.many.works({
	from: r.orders.id,
	to: r.works.orderId
}) } }));
defineRelationsPart(db_exports);
var relations = { ...main };
...
var NotFoundError = class extends RadioError {
...
var upsertCustomer = async ({ name }) => {
	const [customer] = await db.insert(customers).values({ name }).onConflictDoUpdate({
...
var deleteCustomer = async (id) => {
	...
	if (!query) throw new NotFoundError("Customer", id);
	await db.delete(customers).where(eq(customers.id, id));
};
var stations_exports = /* @__PURE__ */ __exportAll({
	attachChannelToStation: () => attachChannelToStation,
	createStation: () => createStation,
	detachChannelStation: () => detachChannelStation,
...
async function createStation(data) {
	return db.transaction(async (tx) => {
		const [station] = await tx.insert(stations).values({
			...
			latitude: data.latitude !== void 0 ? String(data.latitude) : null,
			longitude: data.longitude !== void 0 ? String(data.longitude) : null,
			mode: data.mode
		}).returning();
...
async function updateStation(data) {
	return db.transaction(async (tx) => {
		const [station] = await db.select().from(stations).where(eq(stations.id, data.id)).limit(1);
		if (!station) throw new NotFoundError("Station", data.id);
...
async function attachChannelToStation({ stationId, channelId, order, alias }) {
	const [result] = await db.insert(stationChannels).values({
		channelId,
		stationId,
		alias,
		position: order
...
async function detachChannelStation({ stationId, channelId }) {
	await db.delete(stationChannels).where(and(eq(stationChannels.stationId, stationId), eq(stationChannels.channelId, channelId)));
}
var radio = {
	stations: stations_exports,
	customers: customers_exports
};
//#endregion
export { __exportAll, radio };
